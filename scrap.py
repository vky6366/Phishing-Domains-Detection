...
class web():
    def __init__(self,url):
        ...
        self.soup = self.fetch_website_html(url)
    ...
    def no_of_equals(self):
        no_of_equals = self.domain.count('=')
        return no_of_equals
    
    def no_of_qmark(self):
        no_of_qmark = self.domain.count('?')
        return no_of_qmark
    
    def no_of_ampersand(self):
        no_of_ampersand = self.domain.count('&')
    ...
    def bank(self):
        ...
        if self.soup:
            return 1 if any(keyword in self.url.lower() or keyword in str(self.soup).lower() for keyword in bank_keywords) else 0
        return 0
    ...
    def crypto(self):
        ...
        if self.soup:
            return 1 if any(keyword in self.url.lower() or keyword in str(self.soup).lower() for keyword in crypto_keywords) else 0
    ...
    def digit_ratio_in_url(self):
        return self.no_of_digits / len(self.url)
    ...
    def no_of_obfuscated_char(self):
        obfuscated_chars = ['@', '%', '&', '$', '!', '*', '(', ')', '^', '~', '#', '{', '}', '[', ']']
        self.no_of_obfuscated = sum(self.url.count(char) for char in obfuscated_chars)
        return self.no_of_obfuscated
    ...
    def obfuscation_ratio(self):
        ...
        obfuscation_ratio = self.no_of_obfuscated_char / total_chars if total_chars > 0 else 0
    ...
    def get_url_title_match_score(self):
        if not self.title:
            return 0
        domain = self.domain.netloc.split('.')[-2] if len(self.domain.netloc.split('.')) > 1 else self.domain.netloc
    ...
    def check_payment_indicators(self):
        ...
        has_https = 1 if self.domain.scheme == "https" else 0
